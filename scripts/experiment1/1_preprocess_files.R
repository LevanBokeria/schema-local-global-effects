...
incoming_files <- list.files('./data/experiment1/',pattern = '*.RDS')
...
for (iFile in incoming_files){
        ...
        # Parse it
        json_decoded <- import(paste0('./data/experiment1/',iFile))
...
if (saveDataCSV){
        ...
        write_csv(int_fb_all_ptp,'./results/experiment1/preprocessed_data/intermediate_feedback_all_ptp.csv')
        write_csv(listings_all_ptp,'./results/experiment1/preprocessed_data/listings_all_ptp.csv')
        write.csv(break_rt_all_ptp,'./results/experiment1/preprocessed_data/break_rt_all_ptp.csv',
                  row.names = F)
