...
qc_check_debrief_and_errors <- import('./results/experiment2/qc_check_sheets/qc_check_debrief_and_errors.xlsx') %>%
        select(-reason) %>%
...
break_rt <- import('./results/experiment2/preprocessed_data/break_rt_all_ptp.csv')
...
qc_check_permutation <- permute_mouse_error_expt2(long_data,
...
if (saveDataCSV){
        ...
        export(qc_table,'./results/experiment2/qc_check_sheets/qc_table.csv')
