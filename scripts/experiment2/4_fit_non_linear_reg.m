...
opts = detectImportOptions('./results/experiment2/mean_by_rep_long_all_types.csv');
opts = setvartype(opts,{'border_dist_closest'},'char');

df = readtable('./results/experiment2/mean_by_rep_long_all_types.csv',opts);

% Load the qc pass data
opts = detectImportOptions('./results/experiment2/qc_check_sheets/qc_table.csv');
opts = setvartype(opts,{'qc_fail_overall'},'logical');

qc_pass = readtable('./results/experiment2/qc_check_sheets/qc_table.csv',opts);
...
%% Save the table
if saveData
    writetable(tbl,'./results/experiment2/learning_rate_fits_matlab.csv');
end
